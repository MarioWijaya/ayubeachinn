<div class="w-full space-y-6 pb-10">
  ...
  <x-page-header
    title="Ubah User"
    subtitle="Perbarui informasi user dan status aktif."
  >
    <x-slot:rightSlot>
      <a
        wire:navigate
  ...
  <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition">
    <div class="border-b border-slate-200 bg-slate-50 px-5 py-4">
      ...
      <div class="mt-0.5 text-xs text-slate-500">Password opsional (isi hanya jika ingin mengganti).</div>
    </div>

    <form method="POST" action="{{ route('owner.users.update', $user->id) }}" class="space-y-8 p-5 sm:p-6">
      @csrf
      @method('PUT')
